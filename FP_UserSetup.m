...
%UserVals.personalpath = '/Users/amywolff/Library/CloudStorage/GoogleDrive-awolff@umn.edu' ; %path from your computer to googledrive
UserVals.personalpath ='/Users/amywolff/Library/CloudStorage/GoogleDrive-awolff@umn.edu' ;% 'G:\' ; %path from your computer to googledrive

%% Load file lists
listofloadfiles = false; %set to false if you only have 1 loadfile
loadfilelist = '/Users/amywolff/Downloads/GS_loadfilelist_dSucrCond_Box9_10_redLight copy';%name of single loadfile or list of loadfiles(including path)
...
%% Save locations
UserVals.saveloc = ['Shared drives\AmyWolff\MatlabTemp\Gretchen'];%'G:\Shared drives\LivEngel\Proj_OptoPav\Matlab'];%path to folder to save mat files and figures to - if empty ([]) saves to location of raw data
...
UserVals.signallabel = 'rdLight'; %name of signal type used in plotting
UserVals.N = 25; %multiplicative for downsampling (100 = ~10hz, 25 = ~40Hz) 
UserVals.eventtypes = {'Cue'}; %labels for PC channels including any to be generated as part of option analyses below

UserVals.hideplots = true; % set this to false if you want plots to pop up while analysis is running (slower)
UserVals.trialplots = false; %set to true to plot individual trial raw data - should create these at least once to be able to view trial x trial data
...
UserVals.FilterEvents = false; %set to true to filter an event channel to remove events that are longer or shorter than a given duration
    UserVals.FiltEvent.Chans = [4,1];%reference to eventtypes position of channel/s to filter
...
UserVals.locobouts = false;
    ...
    UserVals.locobout.FPwin = [-1 3]; %time pre and post bout onset to plot for FP analysis
    UserVals.locobout.FPpeakwins = {[0 3],'Post'}; % windows to use for peak analysis for group data (relative to t0) [time start time end},'label'.....
...
UserVals.freezingbouts= false; % set to true to assess freezing with DLC analysis results
    ...
    UserVals.freezebout.FPwin = [-1 3]; %time pre and post bout onset to plot for FP analysis
    UserVals.freezebout.FPpeakwins = {[0 3],'Post'}; % windows to use for peak analysis for group data (relative to t0) format for entry{[time start time end},'label',[time start time end],'label'.....};
...
UserVals.approaches = false; %set to true to measure approaches to a part of the environment
    ...
    UserVals.approach.FPwin = [-1 3]; %time pre and post bout onset to plot for FP analysis
    UserVals.approach.FPpeakwins = {[0 3],'Post'}; % windows to use for peak analysis for group data (relative to t0) [time start time end},'label'.....
 ...
 UserVals.orienting = false; %set to true to measure approaches to a part of the environment
    ...
    UserVals.orient.FPwin = [-1 3]; %time pre and post bout onset to plot for FP analysis
    UserVals.orient.FPpeakwins = {[0 3],'Post'}; % windows to use for peak analysis for group data (relative to t0) [time start time end},'label'.....
