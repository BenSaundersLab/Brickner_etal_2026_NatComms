function [trials]= FP_MaxResponse(timearray,traces,searchwin,peakwin)
...
trials.minval = NaN(length(traces(:,1)),1);
...
endtime = trials.peaktimearray(end);
     ...
     [trials.peakval,trials.peakindx] = max(peakwin,[],2); %finds the peak within  window     
     ...
     [~,risestart] = min(trials.peaktrace(i,1:trials.peakindx(i)));%if slope values before peak are all above 50% set min timepoint between start of window and peak as start of slope measurement
     else
     ...
     if isempty(fallend)
     [~,fallend] = min(trials.peaktrace(i,trials.peakindx(i):end)); %if values post peak don't decay to 50% set slope measurement to min point between peak and end of window
     ...
     %norm traces
     for i =1:length(trials.peakindx) % for each trial
     %find min val between start of window and peak to normalize traces
     [trials.minval(i),~] = min(trials.peaktrace(i,1:trials.peakindx(i))); %find min val for this trial (between start of window and peak)
     %mintime(i) = trials.peaktimearray(minindx(i));
        
     if ~isnan(trials.minval(i))
     trials.normval(i) = 0-trials.minval(i); %get value for normalization of data
     ...
     risestart = find(slopearray < trials.peakindx(i), 1,'last');% find indx of the last value below 50% of peak that occurs before peak
     if isempty(risestart)
     [~,risestart] = min(trials.peaktrace_norm(i,1:trials.peakindx(i)));%if slope values before peak are all above 50% set min timepoint between start of window and peak as start of slope measurement
     else
     risestart = slopearray(risestart)+1; %set first val > 50% as start of slope measurement
     end
     ...
     fallend = find(slopearray > trials.peakindx(i), 1,'first');% find indx of the first value below 50% of peak that occurs after peak
     if isempty(fallend)
     [~,fallend] = min(trials.peaktrace_norm(i,trials.peakindx(i):end)); %if values post peak don't decay to 50% set slope measurement to end at min value between peak and end of window
     reftraceindx = trials.peakindx(i): length(trials.peaktrace(i,:));
     fallend = reftraceindx(fallend);
     else
     fallend = slopearray(fallend)-1;%set last val > 50% as end of slope measurement;
